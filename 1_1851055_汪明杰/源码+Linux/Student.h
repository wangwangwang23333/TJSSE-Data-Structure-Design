...
#ifndef _STUDENTS_H__
#define _STUDENTS_H__
...
public:
	Student() : _id(0), _name(), _gender(male), _age(0), _type() {}
	Student(int id) : _id(id), _name(), _gender(male), _age(0), _type() {}
	int getId() const { return _id; }

	friend istream &operator>>(istream &is, Student &elem);
	friend ostream &operator<<(ostream &os, const Student &elem);

	friend void input_no_id(istream &is, Student &elem);
...
istream &operator>>(istream &is, Student &elem)
{
	string istr;
	...
	is >> elem._age >> elem._type;

	return is;
}

void input_no_id(istream &is, Student &elem)
{
	string istr;
...
ostream &operator<<(ostream &os, const Student &elem)
{
